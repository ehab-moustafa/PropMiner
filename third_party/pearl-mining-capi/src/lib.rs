...
///   3 — added unkeyed BLAKE3 hash and XOF helpers.
#[no_mangle]
...
/// Standard (unkeyed) BLAKE3 hash of an arbitrary byte buffer.
///
/// Matches `Blake3.Hash(input)` in the C# reference. Used for job-key
/// derivation and noise-seed derivation. A zero-keyed BLAKE3 is NOT the
/// same as an unkeyed hash because the key flag is set, so this export is
/// required for byte-for-byte compatibility.
///
/// `data_ptr` may be NULL iff `data_len == 0`. `out` must be 32 bytes.
#[no_mangle]
pub unsafe extern "C" fn pearl_capi_blake3_hash(
    data_ptr: *const u8,
    data_len: usize,
    out: *mut u8,
    err_msg_ptr: *mut *mut c_char,
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| -> Result<(), String> {
        if out.is_null() {
            return Err("out must be non-null".into());
        }
        if data_len > 0 && data_ptr.is_null() {
            return Err("data_ptr null with non-zero data_len".into());
        }
        ...
        std::ptr::copy_nonoverlapping(digest.as_bytes().as_ptr(), out, 32);
        Ok(())
    }));
    match result {
        Ok(Ok(())) => PEARL_CAPI_OK,
        Ok(Err(e)) => { set_err(err_msg_ptr, e); PEARL_CAPI_ERR_BAD_ARG }
        Err(_)     => { set_err(err_msg_ptr, "panic during blake3_hash"); PEARL_CAPI_ERR_INTERNAL }
    }
}

/// BLAKE3 XOF output for the audit_proof v1 index derivation.
///
/// Writes exactly `out_len` bytes of BLAKE3 XOF output from `data`.
/// `data_ptr` may be NULL iff `data_len == 0`. `out` must be non-null.
#[no_mangle]
pub unsafe extern "C" fn pearl_capi_blake3_xof(
    data_ptr: *const u8,
    data_len: usize,
    out: *mut u8,
    out_len: usize,
    ...
    match result {
        Ok(Ok(())) => PEARL_CAPI_OK,
        Ok(Err(e)) => { set_err(err_msg_ptr, e); PEARL_CAPI_ERR_BAD_ARG }
        Err(_)     => { set_err(err_msg_ptr, "panic during blake3_xof"); PEARL_CAPI_ERR_INTERNAL }
    }
}
