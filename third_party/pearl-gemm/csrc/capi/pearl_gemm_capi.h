...
// Fail fast when PEARL_GEMM_KERNEL requests a kernel not compiled in.
int pearl_capi_validate_kernel_selection(void);
...
// Extended CUDA-graph batch path with a caller-owned device-side seed pointer.
//
// The standard graph prepare captures a host->device seed copy, so the seed
// value cannot be uploaded until the graph starts replaying.  The extended
// variant accepts a device pointer `seed_lo_dev` that is read directly by the
// captured LCG fill kernel.  The caller is responsible for populating
// `seed_lo_dev` (e.g. with cudaMemcpyAsync from a pinned host seed ring) and
// for ordering the upload before `pearl_capi_iter_batch_graph_launch_ex` via
// stream waits/events.  This enables the PCIe Gen5 conveyor belt: the CPU
// uploads the next batch's seed while the Tensor Cores are still crunching the
// previous batch.
int pearl_capi_iter_batch_graph_prepare_ex(
    void*         workspace,
    void* const*  host_signal_header_pinned_batch,
    int32_t       count,
    void*         seed_lo_dev,
...
int pearl_capi_iter_batch_graph_launch_ex(
    void*     workspace,
    void*     stream);
