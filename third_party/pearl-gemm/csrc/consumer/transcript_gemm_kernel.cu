...
static_assert(kBM == 128 && kBN == 256,
              "RTX 5090 cache-aligned CTA tile: 128x256 accumulators (128 KiB) "
              "plus pipelined A/B smem tiles are sized for the Blackwell "
...
// TiledMMA — the Tile<> argument's K dim is the MMA *atom* K (= 32), NOT the
// smem kBK.  partition_A/_B/partition_fragment_A/_B then produce
// MMA_K = kBK / kAtomK fragments per smem stage.
...
__global__ void transcript_gemm_kernel_consumer(
  ...
  // Buffers are allocated with 256-byte alignment via cuMemAlloc / cudaMalloc;
