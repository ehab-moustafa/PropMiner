...
uint32_t ptx_rightrotate32(uint32_t x, unsigned int n) {
  uint32_t r;
  // shf.l.wrap.b32 does LEFT rotate: rotl(x, n) = (x << n) | (x >> (32-n)).
  // For RIGHT rotate by n we need rotl(x, 32-n).
...
__global__ void transcript_finalize_kernel(
  ...
  // Load 4 consecutive uint128_t values (16 uint32_t = 64 bytes) directly from gmem.
  const uint128_t* src = reinterpret_cast<const uint128_t*>(transcript + tx_idx);
  uint128_t tmp[4];
  CUTLASS_PRAGMA_UNROLL
  for (int i = 0; i < 4; ++i) {
    tmp[i] = src[i];
