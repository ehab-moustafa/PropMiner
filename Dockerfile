...
RUN --mount=type=cache,id=propminer-ccache,target=/ccache \
    ...
    && cmake --build build_runtime --target propminer propminer_ref_tests -j"$(nproc)" \
    && ./build_runtime/propminer_ref_tests \
    && ccache -s
...
ENV PROPMINER_STRATUM_DIFF=32768
ENV PROPMINER_N_CAP=131072
